...
CREATE TABLE IF NOT EXISTS firecallsgroupcleaned
FROM firecallsclean
...
WHERE `call_type_group`<>'Potentially Life-Threatening' AND `call_type_group`<>'Non Life-threatening'
...
CREATE TEMPORARY VIEW firecallsdf AS
SELECT Call_Type,
...
-- MAGIC %python
-- MAGIC df = spark.createDataFrame(X_test)
-- MAGIC df.createOrReplaceTempView("testTable")
